...
// greater than revision
func (r ApiListSitesInVPCRequest) GtRevision(gtRevision int32) ApiListSitesInVPCRequest {
	r.gtRevision = &gtRevision
	return r
}

func (r ApiListSitesInVPCRequest) Execute() ([]ModelsSite, *http.Response, error) {
	return r.ApiService.ListSitesInVPCExecute(r)
}

/*
ListSitesInVPC List Sites

Lists all sites for this VPC

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param id VPC ID
	@return ApiListSitesInVPCRequest
*/
func (a *VPCApiService) ListSitesInVPC(ctx context.Context, id string) ApiListSitesInVPCRequest {
	return ApiListSitesInVPCRequest{
		ApiService: a,
		ctx:        ctx,
		id:         id,
...
//
//	@return []ModelsSite
func (a *VPCApiService) ListSitesInVPCExecute(r ApiListSitesInVPCRequest) ([]ModelsSite, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "VPCApiService.ListSitesInVPC")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/vpcs/{id}/sites"
	localVarPath = strings.Replace(localVarPath, "{"+"id"+"}", url.PathEscape(parameterValueToString(r.id, "id")), -1)
