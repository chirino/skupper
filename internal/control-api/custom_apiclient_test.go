...
func TestWithTokenFileOption(t *testing.T) {
	...
	_, err = control_api.NewAPIClientWithOptions(context.Background(), mockServer.URL, nil,
		control_api.WithPasswordGrant("fake", "password"),
		control_api.WithTokenStore(store),
	)
	require.NoError(err)
	...
	c, err := control_api.NewAPIClientWithOptions(context.Background(), mockServer.URL, nil,
		control_api.WithPasswordGrant("fake", "password"),
		control_api.WithTokenStore(store),
	)
...
func addMockOIDCRoutes(server *httptest.Server, router *http.ServeMux, createAccessToken func() string) error {
	...
	router.HandleFunc("/device/login/start", func(resp http.ResponseWriter, req *http.Request) {
		sendJson(resp, http.StatusOK, control_api.ModelsDeviceStartResponse{
			ClientId:                    control_api.PtrString("nexodus-cli"),
			DeviceAuthorizationEndpoint: control_api.PtrString(server.URL + "/realms/nexodus/protocol/openid-connect/auth/device"),
			Issuer:                      control_api.PtrString(server.URL + "/realms/nexodus"),
		})
	})
