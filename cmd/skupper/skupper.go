...
installation that can then be connected to other skupper installations`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if initFlags.regKey != "" {
				err := registerWithControlPlane(&initFlags, &routerCreateOpts, skupperCli.Platform())
				if err != nil {
					return err
				}
			}
	...
	// hidden control plane flags
	cmd.Flags().StringVarP(&initFlags.regKey, "reg-key", "", "", "Registration key used to join a managed Skupper network")
	cmd.Flags().MarkHidden("reg-key") // hide this flag until it is fully implemented
	cmd.Flags().StringVarP(&initFlags.controlPlaneAddress, "control-plane-address", "", "", "The IP or hostname of the control plane.  If not specified, the URL in the reg key will be used.")
	cmd.Flags().MarkHidden("control-plane-address") // hide this flag until it is fully implemented
	cmd.Flags().BoolVarP(&initFlags.insecureSkipTlsVerify, "insecure-skip-tls-verify", "", false, "If true, server certificates will not be checked for validity. This will make your HTTPS connections insecure")
	cmd.Flags().MarkHidden("insecure-skip-tls-verify") // hide this flag until it is fully implemented
...
func NewCmdExpose(skupperCli SkupperServiceClient) *cobra.Command {
	...
	cmd.Flags().BoolVar(&exposeOpts.GeneratedCerts, "enable-tls", false, "If specified, the service will be exposed over TLS")
	cmd.Flags().BoolVar(&exposeOpts.GeneratedCerts, "generate-tls-secrets", false, "If specified, the service will be exposed over TLS")
...
func NewCmdCreateService(skupperClient SkupperServiceClient) *cobra.Command {
	...
	cmd.Flags().StringVar(&serviceToCreate.TlsCredentials, "tls-cert", "", "K8s secret name with custom certificates to encrypt the communication using TLS")
...
func NewCmdBind(skupperClient SkupperServiceClient) *cobra.Command {
	...
	cmd.Flags().StringVar(&bindOptions.tlsCertAuthority, "tls-trust", "", "K8s secret name with the CA to expose the service over TLS")
